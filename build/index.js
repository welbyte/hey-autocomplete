...
%o`, message, error2, detail);
  ...
  // lib/autocomplete/util.js
  function memoize(object, name, value) {
  ...
  var Record = class {
    ...
    score(query) {
      ...
      if (this.string == query)
        return 1;
      if (this.string.startsWith(query))
        return 2;
      if (this.parts.some((part) => part == query))
        return 3;
      if (this.parts.some((part) => part.startsWith(query)))
        return 4;
      if (this.string.includes(query))
        return 5;
    }
    get values() {
  ...
  // lib/autocomplete/collection.js
  var _Collection = class {
    static async fetch(url, { expireIn = 60 * 1e3 } = {}) {
      let collection = this.index.get(url);
      if (!collection) {
        collection = _Collection.load(url);
    ...
    static async load(url) {
      return _Collection.fromJSON(await getJSON(url));
    }
    static fromJSON(json) {
      return new _Collection(json.map(Record.fromJSON));
    }
    ...
    add(value) {
      ...
      const { records } = new _Collection(this.records.concat(record));
      this.records = records;
      return record;
    }
    without(...values) {
      ...
      return new _Collection(records, { sort: false });
    }
    first(limit) {
    ...
    search(query) {
      ...
      return new _Collection(records.flat(), { sort: false });
    }
    get length() {
  ...
  __publicField(Collection, "index", /* @__PURE__ */ new Map());

  // lib/autocomplete/selection.js
  var Selection = class {
    ...
    get template() {
      const id2 = this.element.getAttribute("data-template-id");
      return memoize(this, "template", document.getElementById(id2));
    }
  };

  // lib/autocomplete/list.js
  var id = 0;
  var List = class {
    constructor(combobox, element) {
      this.combobox = combobox;
  ...
  var Input = class {
    ...
    set activeDescendant(element) {
      ...
      if (id2)
        this.element.setAttribute("aria-activedescendant", id2);
      else
        this.element.removeAttribute("aria-activedescendant");
    }
  };

  // lib/autocomplete/combobox.js
  var Combobox = class {
    ...
    update() {
      this.list.setCollection(
        this.collection.without(...this.selection.values).search(this.input.value).first(20)
      );
  ...
  var autocomplete_controller_default = class extends Controller {
    ...
    copySelected(event) {
      if (this.activeElementIsInScope && this.selectedValue && event.clipboardData) {
    ...
    parsers = {
      email(string = "") {
        ...
        for (const part of string.split(/,|\n/)) {
          ...
          if (value)
            values.push(value.replace(/^mailto:/, ""));
        }
        return values;
      }
    };
  };
  __publicField(autocomplete_controller_default, "targets", ["input", "list", "select"]);
  __publicField(autocomplete_controller_default, "values", { url: String, forbidAdding: Boolean });
